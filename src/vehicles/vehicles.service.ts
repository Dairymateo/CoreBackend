...
export class VehiclesService {
  ...
  async update(id: string, updateVehicleDto: UpdateVehicleDto): Promise<VehicleDocument | null> {
    try {
      const updatedVehicleInfo = await this.vehicleModel.findByIdAndUpdate(id, updateVehicleDto, { new: true });
      ...
      if (fullUpdatedVehicle) {
        fullUpdatedVehicle.vehiclePerfomance = this.CalculateVehiclePerformance(fullUpdatedVehicle);
  ...
  CalculateVehiclePerformance(vehicle: VehicleDocument): number {
    ...
    const rendimientoV = (velocidadP * 0.4) + ((1 - tasaDeFallo) * 0.4) + ((1 - pesoNormalizado) * 0.2);
    return rendimientoV;
  }
}
