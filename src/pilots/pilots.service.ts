...
export class PilotsService {
  ...
  async create(createPilotDto: CreatePilotDto) {
    const vehicle = await this.vehiclesService.findOne(createPilotDto.vehiculoId);
    if (!vehicle) {
      throw new NotFoundException(`Vehicle with id "${createPilotDto.vehiculoId}" not found`);
    }
  ...
  async update(id: string, updatePilotDto: UpdatePilotDto): Promise<PilotDocument | null> {
    try {
      ...
      if (updatePilotDto.vehiculoId) {
        const vehicle = await this.vehiclesService.findOne(updatePilotDto.vehiculoId);
        if (!vehicle) {
          throw new NotFoundException(`Vehicle with id "${updatePilotDto.vehiculoId}" not found`);
        }
        updatePilotDto.vehiculoId = vehicle._id.toString(); 
      }
  
      const updatedPilotInfo = await this.pilotModel.findByIdAndUpdate(id, updatePilotDto, { new: true }).exec();
  
      if (updatedPilotInfo) {
        updatedPilotInfo.generalPerfomance = this.calculateGeneralPilotPerformance(updatedPilotInfo as PilotDocument);
  ...
  calculateGeneralPilotPerformance(pilot: PilotDocument): number{
   ...
   calculateFinalPerformance(pilot: PilotDocument, ciruit: CircuitDocument): number {
    ...
    const finalPerformance = (pilotPerformance * pesoP) + (vehiclePerformance * pesoV);

    return finalPerformance;
  }

  async getPilotRankingForCircuit(circuitId: string): Promise<{ pilot: PilotDocument; finalPerformance: number}[]>{
    ...
    const pilotPerformances = await Promise.all(
      pilots.map(async (pilot) => {
        const finalPerformance = await this.calculateFinalPerformance(pilot, circuit);
